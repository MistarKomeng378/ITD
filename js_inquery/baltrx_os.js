...
function create_baltrx_os_event()
{
    ...
    $("#hbtn_baltrx_os_up").click(function(){
        if(confirm('Upload Data?')) $('#frm_baltrx_os').submit(); 
    });
               
}
function baltrx_os_initiate_upload()
{
    $('#frm_baltrx_os').submit(function(){
        var response;
        state_progress(1); 
        var frame_du=$("#iframe_baltrx_os").load(function(){
            response=frame_du.contents().find('body');
            if(response.html()!='') alert(response.html());
