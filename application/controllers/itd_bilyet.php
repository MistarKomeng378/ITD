...
class Itd_bilyet extends CI_Controller { 
    var $isLogin;
    var $data;          
    ...
    function update_bilyet_inst()
    {
        $param=$this->input->post();
        sql_quot_all($param);

        $this->load->model("M_itd_bilyet");    
        $data = $this->M_itd_bilyet->update_bilyet_inst(trim($param["trx_id"]),trim($param["bil_no"]),trim($param["bil_desc"]),$this->data["uid"]);
        $this->data["r_success"] = 1;
        echo json_encode($this->data);
    } 
    ...
    function do_bilyet_inst_cancel()
    {
        $param=$this->input->post();
        sql_quot_all($param);
        
        $this->load->model("M_itd_bilyet");    
        $data = $this->M_itd_bilyet->do_bilyet_inst_cancel(trim($param["trx_id"]),0+trim($param["i_c"]),$this->data["uid"]);
        $this->data["r_success"] = 1;
        echo json_encode($this->data);
    }
    function save_list_bilyet()
    {
        $param=$this->input->post();
        $this->load->model("M_itd_bilyet");    
        $param["b_val_dt"]=change_dt_format($param["b_val_dt"]);
        ...
        if(0+$param["tipe"]==0)
            ...
            $data = $this->M_itd_bilyet->list_pending_bilyet_inst($param["p1"],$param["p2"]);
        $fields = array("trx_valuta_date","trx_due_date","dt_in","dt_in1","dt_out");
        add_data_dt_str($data,$fields);
        $this->data["r_success"] = 1;
        $this->data["r_num_rows"] = count($data);
