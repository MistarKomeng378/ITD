<?php
class M_subsrd_client extends CI_Model {
    ...
    function list_client($c_code='',$c_enable=0)
    {
        $query=$this->db_jasgir->query("exec subsrd_list_client '{$c_code}',{$c_enable}");
        $data=$query->result_array();
        return $data;
    }
    
    function add_client($b_ccode='',$b_cname='',$b_micode='',$b_enable=1,$user_id='')
    { //echo "exec [add_trx] '{$c_code}','{$c_no}','{$c_dt}','{$c_catcode}','{$c_catdesc}','{$c_desc}','{$c_dc}',{$c_nominal},'{$user_id}'";
        $query=$this->db_jasgir->query("exec [subsrd_add_client] '{$b_ccode}','{$b_cname}','{$b_micode}',{$b_enable},'{$user_id}'");
        $data=$query->result_array();
        return $data;
    }
    function edit_client($b_ccode='',$b_cname='',$b_micode='',$b_enable=1,$user_id='')
    { ////echo "exec [subs_edit_client] {$b_id},'{$b_cname}','{$b_bname}','{$user_id}'";
        $query=$this->db_jasgir->query("exec [subsrd_edit_client] '{$b_ccode}','{$b_cname}','{$b_micode}',{$b_enable},'{$user_id}'");
        $data=$query->result_array();
        return $data;
    }
    ...
    function get_client($b_id='')
    { //echo "exec [add_trx] '{$c_code}','{$c_no}','{$c_dt}','{$c_catcode}','{$c_catdesc}','{$c_desc}','{$c_dc}',{$c_nominal},'{$user_id}'";
        $query=$this->db_jasgir->query("exec [subsrd_get_client] '{$b_id}'");
        $data=$query->result_array();
        return $data;
    }
