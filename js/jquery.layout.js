...
return fn(pane,$Ps[pane],state[pane],options[pane],options.name);else
return fn(Instance,state,options,options.name);}}
catch(ex){}},_fixIframe=function(pane){if($.layout.browser.mozilla)return;var $P=$Ps[pane];if(state[pane].tagName=="IFRAME")
$P.css(_c.hidden).css(_c.visible);else
$P.find('IFRAME').css(_c.hidden).css(_c.visible);},cssW=function(el,outerWidth){var str=isStr(el),$E=str?$Ps[el]:$(el);if(!$E.length)return 0;if(isNaN(outerWidth))
outerWidth=str?getPaneSize(el):$E.outerWidth();return $.layout.cssWidth($E,outerWidth);},cssH=function(el,outerHeight){var str=isStr(el),$E=str?$Ps[el]:$(el);if(!$E.length)return 0;if(isNaN(outerHeight))
outerHeight=str?getPaneSize(el):$E.outerHeight();return $.layout.cssHeight($E,outerHeight);},cssSize=function(pane,outerSize){if(_c[pane].dir=="horz")
return cssH(pane,outerSize);else
return cssW(pane,outerSize);},cssMinDims=function(pane){var
...
$E.hide().data('autoHidden',true);},setOuterHeight=function(el,outerHeight,autoHide){var $E=el,h;if(isStr(el))$E=$Ps[el];else if(!el.jquery)$E=$(el);h=cssH($E,outerHeight);$E.css({height:h,visibility:"visible"});if(h>0&&$E.innerWidth()>0){if(autoHide&&$E.data('autoHidden')){$E.show().data('autoHidden',false);if(!$.layout.browser.mozilla)
$E.css(_c.hidden).css(_c.visible);}}
else if(autoHide&&!$E.data('autoHidden'))
...
return 0;else{var
$P=$Ps[pane],dim=(dir=="horz"?"height":"width"),vis=$.layout.showInvisibly($P),s=$P.css(dim);;$P.css(dim,"auto");size=(dim=="height")?$P.outerHeight():$P.outerWidth();$P.css(dim,s).css(vis);return size;}},getPaneSize=function(pane,inclSpace){var
...
evt.stopPropagation();$E.addClass(getHoverClasses($E));},removeHover=function(evt,el){var $E=$(el||this);$E.removeClass(getHoverClasses($E,true));},onResizerEnter=function(evt){$('body').disableSelection();addHover(evt,this);},onResizerLeave=function(evt,el){var
...
$('body').enableSelection();},_create=function(){initOptions();var o=options;$.layout.browser.boxModel=$.support.boxModel;if(o.useStateCookie&&o.cookie.autoLoad)
loadCookie();state.creatingLayout=true;if(false===_execCallback(null,o.onload_start))
return'cancel';_initContainer();initHotkeys();if(o.autoBindCustomButtons)initButtons();$(window).bind("unload."+sID,unload);if(o.initPanes)_initLayoutElements();delete state.creatingLayout;return state.initialized;},isInitialized=function(){if(state.initialized||state.creatingLayout)return true;else return _initLayoutElements();},_initLayoutElements=function(){var o=options;if(!$N.is(":visible"))
return false;if(!getPane('center').length){if(o.showErrorMessages)alert(lang.errCenterPaneMissing);return false;}
state.creatingLayout=true;$.extend(sC,elDims($N));initPanes();sizeContent();if(o.scrollToBookmarkOnLoad){var l=self.location;if(l.hash)l.replace(l.hash);}
if(o.resizeWithWindow&&!$N.data("layoutRole"))
$(window).bind("resize."+sID,windowResize);delete state.creatingLayout;state.initialized=true;_execCallback(null,o.onload_end||o.onload);return true;},windowResize=function(){var delay=Number(options.resizeWithWindowDelay);if(delay<10)delay=100;timer.clear("winResize");timer.set("winResize",function(){timer.clear("winResize");timer.clear("winResizeRepeater");var dims=elDims($N);if(dims.innerWidth!==sC.innerWidth||dims.innerHeight!==sC.innerHeight)
...
timer.set("winResizeRepeater",function(){setWindowResizeRepeater();resizeAll();},delay);},unload=function(){var o=options;state.cookie=getState();_execCallback(null,o.onunload_start);if(o.useStateCookie&&o.cookie.autoSave)saveCookie();_execCallback(null,o.onunload_end||o.onunload);},_initContainer=function(){var
tag=sC.tagName=$N[0].tagName,o=options,fullPage=(tag=="BODY"),props="overflow,position,margin,padding,border",CSS={},hid="hidden",isVis=$N.is(":visible");sC.selector=$N.selector.split(".slice")[0];sC.ref=tag+"/"+sC.selector;$N.data("layout",Instance).data("layoutContainer",sID).addClass(o.containerClass);if(!$N.data("layoutCSS")){if(fullPage){CSS=$.extend(elCSS($N,props),{height:$N.css("height"),overflow:$N.css("overflow"),overflowX:$N.css("overflowX"),overflowY:$N.css("overflowY")});var $H=$("html");$H.data("layoutCSS",{height:"auto",overflow:$H.css("overflow"),overflowX:$H.css("overflowX"),overflowY:$H.css("overflowY")});}
else
CSS=elCSS($N,props+",top,bottom,left,right,width,height,overflow,overflowX,overflowY");$N.data("layoutCSS",CSS);}
...
var
p=$N.css("position"),h=$N.css("height");if(!$N.data("layoutRole")){if(!p||!p.match(/fixed|absolute|relative/))
CSS.position="relative";}
$N.css(CSS);if(isVis){$.extend(sC,elDims($N));if(o.showErrorMessages&&sC.innerHeight<2)
...
fx=effects[fxName]||{},fx_all=fx.all||{},fx_pane=fx[pane]||{};o[sSettings]=$.extend({},fx_all,fx_pane,defs.fxSettings||{},defs[sSettings]||{},o.fxSettings,o[sSettings],opts.defaults.fxSettings,opts.defaults[sSettings]||{},opts[pane].fxSettings,opts[pane][sSettings]||{});o[sSpeed]=opts[pane][sSpeed]||opts[pane].fxSpeed||opts.defaults[sSpeed]||opts.defaults.fxSpeed||o[sSpeed]||o[sSettings].duration||o.fxSpeed||o.fxSettings.duration||defs.fxSpeed||defs.fxSettings.duration||fx_pane.duration||fx_all.duration||"normal";});});function renameOpts(O){for(var key in newOpts){if(O[key]!=undefined){O[newOpts[key]]=O[key];delete O[key];}}}},getPane=function(pane){var sel=options[pane].paneSelector
if(sel.substr(0,1)==="#")
return $N.find(sel).eq(0);else{var $P=$N.children(sel).eq(0);return $P.length?$P:$N.children("form:first").children(sel).eq(0);}},initPanes=function(){$.each(_c.allPanes.split(","),function(idx,pane){addPane(pane,true);});initHandles();$.each(_c.borderPanes.split(","),function(i,pane){if($Ps[pane]&&state[pane].isVisible){setSizeLimits(pane);makePaneFit(pane);}});sizeMidPanes("center");setTimeout(function(){$.each(_c.allPanes.split(","),function(i,pane){var o=options[pane];if($Ps[pane]&&state[pane].isVisible){if(o.triggerEventsOnLoad)
_execCallback(pane,o.onresize_end||o.onresize);resizeNestedLayout(pane);}});},50);if(options.showErrorMessages&&$N.innerHeight()<2)
alert(lang.errContainerHeight.replace(/CONTAINER/,sC.ref));},addPane=function(pane,force){if(!force&&!isInitialized())return;var
...
s.tagName=$P[0].tagName;s.edge=pane
...
CSS.width=max(1,cssW(pane,size));$P.css(CSS);if(dir!="horz")sizeMidPanes(pane,true);if(o.initClosed&&o.closable&&!o.initHidden)
...
_execCallback(pane,o.onresize_end||o.onresize);resizeNestedLayout(pane);}}},initHandles=function(panes){if(!panes||panes=="all")panes=_c.borderPanes;$.each(panes.split(","),function(i,pane){var $P=$Ps[pane];$Rs[pane]=false;$Ts[pane]=false;if(!$P)return;var
...
setAsOpen(pane);else{setAsClosed(pane);bindStartSlidingEvent(pane,true);}});sizeHandles("all");},initContent=function(pane,resize){if(!isInitialized())return;var
...
$Cs[pane]=false;},initButtons=function(){var pre="ui-layout-button-",name;$.each("toggle,open,close,pin,toggle-slide,open-slide".split(","),function(i,action){$.each(_c.borderPanes.split(","),function(ii,pane){$("."+pre+action+"-"+pane).each(function(){name=$(this).data("layoutName")||$(this).attr("layoutName");if(name==undefined||name==options.name)
bindButton(this,action,pane);});});});},initResizable=function(panes){var
...
$Frames.each(function(){$("#"+$(this).data("layoutMaskID")).css($(this).position()).css({width:this.offsetWidth+"px",height:this.offsetHeight+"px"});});newSize=resizerPos-sC["inset"+c.side];manualSizePane(pane,newSize);}});},destroy=function(){$(window).unbind("."+sID);$(document).unbind("."+sID);$.each(_c.allPanes.split(","),function(i,pane){removePane(pane,false,true);});$N.removeData("layout").removeData("layoutContainer").removeClass(options.containerClass);if(!$N.data("layoutEdge")&&$N.data("layoutCSS"))
$N.css($N.data("layoutCSS")).removeData("layoutCSS");if(sC.tagName=="BODY"&&($N=$("html")).data("layoutCSS"))
$N.css($N.data("layoutCSS")).removeData("layoutCSS");unload();},removePane=function(pane,remove,skipResize){if(!isInitialized())return;if(!$Ps[pane])return;var
...
if($T&&$T.length)$T.remove();if($R&&$R.length)$R.remove();$Ps[pane]=$Cs[pane]=$Rs[pane]=$Ts[pane]=false;if(!skipResize){resizeAll();state[pane]={};}},hide=function(pane,noAnimation){if(!isInitialized())return;var
...
else{s.isHiding=true;close(pane,false,noAnimation);}},show=function(pane,openPane,noAnimation,noAlert){if(!isInitialized())return;var
...
open(pane,false,noAnimation,noAlert);},toggle=function(pane,slide){if(!isInitialized())return;if(!isStr(pane)){pane.stopImmediatePropagation();pane=$(this).data("layoutEdge");}
...
$P=$Ps[pane],s=state[pane];$P.hide();s.isClosed=true;s.isVisible=false;},close=function(pane,force,noAnimation,skipCallback){if(!state.initialized&&$Ps[pane]){_closePane(pane);return;}
...
_dequeue(pane);}},setAsClosed=function(pane){var
...
syncPinBtns(pane,false);if(state.initialized){sizeHandles("all");}},open=function(pane,slide,noAnimation,noAlert){if(!isInitialized())return;var
...
setSizeLimits(pane,slide);if(false===_execCallback(pane,o.onopen_start))return;setSizeLimits(pane,slide);if(s.minSize>s.maxSize){syncPinBtns(pane,false);if(!noAlert&&o.noRoomToOpenTip)
alert(o.noRoomToOpenTip);return;}
...
_dequeue(pane);};},setAsOpen=function(pane,skipCallback){var
...
syncPinBtns(pane,!s.isSliding);$.extend(s,elDims($P));if(state.initialized){sizeHandles("all");sizeContent(pane,true);}
if(!skipCallback&&(state.initialized||o.triggerEventsOnLoad)&&$P.is(":visible")){_execCallback(pane,o.onopen_end||o.onopen);if(s.isShowing)_execCallback(pane,o.onshow_end||o.onshow);if(state.initialized){_execCallback(pane,o.onresize_end||o.onresize);resizeNestedLayout(pane);}}},slideOpen=function(evt_or_pane){if(!isInitialized())return;var
...
open(pane,true);};},slideClose=function(evt_or_pane){if(!isInitialized())return;var
...
close(pane);};},slideToggle=function(pane){toggle(pane,true);},lockPaneForFX=function(pane,doLock){var $P=$Ps[pane];if(doLock){$P.css({zIndex:_c.zIndex.pane_animate});if(pane=="south")
...
$P.css({left:"auto"});var o=options[pane];if($.layout.browser.msie&&o.fxOpacityFix&&o.fxName_open!="slide"&&$P.css("filter")&&$P.css("opacity")==1)
$P[0].style.removeAttribute('filter');}},bindStartSlidingEvent=function(pane,enable){var
...
[enable?"bind":"unbind"](trigger+'.'+sID,slideOpen).css("cursor",enable?o.sliderCursor:"default").attr("title",enable?o.sliderTip:"");},bindStopSlidingEvents=function(pane,enable){var
...
side=c.side.toLowerCase(),pos=s.size+sC["inset"+c.side];if($.layout.cssNum($R,side)!=pos)$R.css(side,pos);}
...
hide(pane,true);}}},manualSizePane=function(pane,size,skipCallback){if(!isInitialized())return;var
o=options[pane],forceResize=o.resizeWhileDragging&&!_c.isLayoutBusy;o.autoResize=false;sizePane(pane,size,skipCallback,forceResize);},sizePane=function(pane,size,skipCallback,force){if(!isInitialized())return;var
o=options[pane],s=state[pane],$P=$Ps[pane],$R=$Rs[pane],side=_c[pane].side.toLowerCase(),dimName=_c[pane].sizeType.toLowerCase(),inset="inset"+_c[pane].side,skipResizeWhileDragging=_c.isLayoutBusy&&!o.triggerEventsWhileDragging,oldSize;setSizeLimits(pane);oldSize=s.size;size=_parseSize(pane,size);size=max(size,_parseSize(pane,o.minSize));size=min(size,s.maxSize);if(size<s.minSize){makePaneFit(pane,false,skipCallback);return;}
if(!force&&size==oldSize)return;if(!skipCallback&&state.initialized&&s.isVisible)
_execCallback(pane,o.onresize_start);$P.css(dimName,max(1,cssSize(pane,size)));s.size=size;$.extend(s,elDims($P));if($R&&$P.is(":visible"))$R.css(side,size+sC[inset]);sizeContent(pane);if(!skipCallback&&!skipResizeWhileDragging&&state.initialized&&s.isVisible){_execCallback(pane,o.onresize_end||o.onresize);resizeNestedLayout(pane);}
if(!skipCallback){if(!s.isSliding)sizeMidPanes(_c[pane].dir=="horz"?"all":"center",skipResizeWhileDragging,force);sizeHandles("all");}
var altPane=_c.altSide[pane];if(size<oldSize&&state[altPane].noRoom){setSizeLimits(altPane);makePaneFit(altPane,false,skipCallback);}},sizeMidPanes=function(panes,skipCallback,force){if(!panes||panes=="all")panes="east,west,center";$.each(panes.split(","),function(i,pane){if(!$Ps[pane])return;var
...
return true;$.extend(s,cssMinDims(pane),{maxWidth:newCenter.width,maxHeight:newCenter.height});CSS=newCenter;CSS.width=cssW(pane,CSS.width);CSS.height=cssH(pane,CSS.height);hasRoom=CSS.width>0&&CSS.height>0;if(!hasRoom&&!state.initialized&&o.minWidth>0){var
...
return true;CSS.top=newCenter.top;CSS.bottom=newCenter.bottom;CSS.height=cssH(pane,newCenter.height);s.maxHeight=max(0,CSS.height);hasRoom=(s.maxHeight>0);if(!hasRoom)s.noVerticalRoom=true;}
...
return true;if(pane=="center"){var b=$.layout.browser;var fix=b.isIE6||(b.msie&&!b.boxModel);if($Ps.north&&(fix||state.north.tagName=="IFRAME"))
$Ps.north.css("width",cssW($Ps.north,sC.innerWidth));if($Ps.south&&(fix||state.south.tagName=="IFRAME"))
$Ps.south.css("width",cssW($Ps.south,sC.innerWidth));}
if(!skipCallback&&state.initialized){_execCallback(pane,o.onresize_end||o.onresize);resizeNestedLayout(pane);}});},resizeAll=function(){if(!state.initialized){_initLayoutElements();return;}
var oldW=sC.innerWidth,oldH=sC.innerHeight;if(!$N.is(":visible:"))return;$.extend(state.container,elDims($N));if(!sC.outerHeight)return;if(false===_execCallback(null,options.onresizeall_start))return false;var
...
sizePane(pane,o.size,true,true);else{setSizeLimits(pane);makePaneFit(pane,false,true,true);}});sizeMidPanes("all",true,true);sizeHandles("all");o=options;$.each(_c.allPanes.split(","),function(i,pane){$P=$Ps[pane];if(!$P)return;if(state[pane].isVisible){_execCallback(pane,o[pane].onresize_end||o[pane].onresize);resizeNestedLayout(pane);}});_execCallback(null,o.onresizeall_end||o.onresizeall);},resizeNestedLayout=function(pane){var
...
$C.layout().resizeAll();}},sizeContent=function(panes,remeasure){if(!isInitialized())return;if(!panes||panes=="all")panes=_c.allPanes;$.each(panes.split(","),function(idx,pane){var
...
m.spaceBelow=_below($C);};});},sizeHandles=function(panes){if(!panes||panes=="all")panes=_c.borderPanes;$.each(panes.split(","),function(i,pane){var
...
$R.show();if(dir=="horz"){paneLen=$P.outerWidth();s.resizerLength=paneLen;$R.css({width:max(1,cssW($R,paneLen)),height:max(0,cssH($R,spacing)),left:$.layout.cssNum($P,"left")});}
...
removeHover(0,$T);}
if(!state.initialized&&(o.initHidden||s.noRoom)){$R.hide();if($T)$T.hide();}});},enableClosable=function(pane){if(!isInitialized())return;var $T=$Ts[pane],o=options[pane];if(!$T)return;o.closable=true;$T.bind("click."+sID,function(evt){evt.stopPropagation();toggle(pane);}).bind("mouseenter."+sID,addHover).bind("mouseleave."+sID,removeHover).css("visibility","visible").css("cursor","pointer").attr("title",state[pane].isClosed?o.togglerTip_closed:o.togglerTip_open).show();},disableClosable=function(pane,hide){if(!isInitialized())return;var $T=$Ts[pane];if(!$T)return;options[pane].closable=false;if(state[pane].isClosed)open(pane,false,true);$T.unbind("."+sID).css("visibility",hide?"hidden":"visible").css("cursor","default").attr("title","");},enableSlidable=function(pane){if(!isInitialized())return;var $R=$Rs[pane],o=options[pane];if(!$R||!$R.data('draggable'))return;options[pane].slidable=true;if(s.isClosed)
bindStartSlidingEvent(pane,true);},disableSlidable=function(pane){if(!isInitialized())return;var $R=$Rs[pane];if(!$R)return;options[pane].slidable=false;if(state[pane].isSliding)
close(pane,false,true);else{bindStartSlidingEvent(pane,false);$R.css("cursor","default").attr("title","");removeHover(null,$R[0]);}},enableResizable=function(pane){if(!isInitialized())return;var $R=$Rs[pane],o=options[pane];if(!$R||!$R.data('draggable'))return;o.resizable=true;$R.draggable("enable").bind("mouseenter."+sID,onResizerEnter).bind("mouseleave."+sID,onResizerLeave);if(!state[pane].isClosed)
...
s.cssSaved=curCSS;$P.css(newCSS);$.each(_c.allPanes.split(","),function(i,p){if(p!=pane)resetOverflow(p);});};function resetOverflow(el){if(!isInitialized())return;if(this&&this.tagName)el=this;var $P;if(isStr(el))
$P=$Ps[el];else if($(el).data("layoutRole"))
$P=$(el);else
$(el).parents().each(function(){if($(this).data("layoutRole")){$P=$(this);return false;}});if(!$P||!$P.length)return;var
pane=$P.data("layoutEdge"),s=state[pane],CSS=s.cssSaved||{};if(!s.isSliding&&!s.isResizing)
$P.css("zIndex",_c.zIndex.pane_normal);$P.css(CSS);s.cssSaved=false;};function getBtn(selector,pane,action){var $E=$(selector),err=options.showErrorMessages;if(!$E.length){if(err)alert(lang.errButton+lang.selector+": "+selector);}
else if(_c.borderPanes.indexOf(pane)==-1){if(err)alert(lang.errButton+lang.pane+": "+pane);}
else{var btn=options[pane].buttonClass+"-"+action;$E.addClass(btn+" "+btn+"-"+pane).data("layoutName",options.name);return $E;}
...
return o;};function loadState(opts,animate){opts=_transformData(opts);$.extend(true,options,opts);if(state.initialized){var pane,o,s,h,c,a=!animate;$.each(_c.allPanes.split(","),function(idx,pane){o=opts[pane];if(typeof o!='object')return;s=o.size;c=o.initClosed;h=o.initHidden;if(s>0||s=="auto")sizePane(pane,s);if(h===true)hide(pane,a);else if(c===false)open(pane,false,a);else if(c===true)close(pane,false,a);else if(h===false)show(pane,false,a);});}};function getState(keys){var
...
catch(e){return{};}};var $N=$(this).eq(0);if(!$N.length){if(options.showErrorMessages)
alert(lang.errContainerMissing);return null;};if($N.data("layoutContainer")&&$N.data("layout"))
return $N.data("layout");var
$Ps={},$Cs={},$Rs={},$Ts={},sC=state.container,sID=state.id;var Instance={options:options,state:state,container:$N,panes:$Ps,contents:$Cs,resizers:$Rs,togglers:$Ts,toggle:toggle,hide:hide,show:show,open:open,close:close,slideOpen:slideOpen,slideClose:slideClose,slideToggle:slideToggle,initContent:initContent,sizeContent:sizeContent,sizePane:manualSizePane,swapPanes:swapPanes,resizeAll:resizeAll,initPanes:isInitialized,destroy:destroy,addPane:addPane,removePane:removePane,setSizeLimits:setSizeLimits,bindButton:bindButton,addToggleBtn:addToggleBtn,addOpenBtn:addOpenBtn,addCloseBtn:addCloseBtn,addPinBtn:addPinBtn,allowOverflow:allowOverflow,resetOverflow:resetOverflow,encodeJSON:encodeJSON,decodeJSON:decodeJSON,getState:getState,getCookie:getCookie,saveCookie:saveCookie,deleteCookie:deleteCookie,loadCookie:loadCookie,loadState:loadState,cssWidth:cssW,cssHeight:cssH,enableClosable:enableClosable,disableClosable:disableClosable,enableSlidable:enableSlidable,disableSlidable:disableSlidable,enableResizable:enableResizable,disableResizable:disableResizable};if(_create()==='cancel')
return null;else
